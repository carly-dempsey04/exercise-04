...
## Assignment 4 - Program a Word Game

Below contains the code to run a game of NYT's Wordle in R

```{r}
#Loading in my libraries needed for the entire project
library(tidyverse)
library(sjmisc)
...
load_dictionary <- function(filename) {
    ...
    wlist <- wlist[["words"]] #providing the column name
}
...
Step 2 - Winnowing the solution list
...
```{r}
#
pick_solution <- function(wlist) {
    solution_options <- wlist[nchar(wlist) == 5] #Only including words that are five characters
...
Step 4 - Play Wordle and Evaluate Guess functions
...
evaluate_guess <- function(guess, coolword) {
    ...
    solution_count <- tibble(letter = coolword) %>%
     ...
     #Joins the two counts by letter and mutates to account for excess guesses
    ...
    #The real pain in this chunk -> supposed to go through letter by letter to double check if player guess matching coolword; I tried so hard and I simply cannot manage
    for (i in 1:word_length) { #using operators to test for letters in guess to letters in coolword
        resulttxt[i] <- case_when(guess[i] %in% coolword & guess[i] == coolword[i] ~ "*", guess[i] %in% coolword & guess[i] != coolword[i] ~ "+", guess[i] %nin% coolword ~ "-")
      #Accounting for the yellow block from wordle, where the letter is present but not in that slot
        for (s in counts$letter) {
            if (guess[i] == s && resulttxt[i] != "*" && counts[counts$letter == s
                ]$to_clear > 0) {
                resulttxt[i] <- "-"
